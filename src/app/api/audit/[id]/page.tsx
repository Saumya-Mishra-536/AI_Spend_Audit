...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description = data.is_optimized
    ...
    : `This team could save $${savings.toFixed(0)}/month on AI tools`;

  return {
    title: `AI Spend Audit - Save $${savings.toFixed(0)}/month`,
    description,
    openGraph: {
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white py-12">
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-4">
            {isOptimized ? (
              ...
              <>
                Save <span className="text-green-600">${totalMonthlySavings.toFixed(0)}/month</span>
              </>
            )}
          </h1>
          {!isOptimized && (
            <p className="text-xl text-slate-600">
              That's <span className="font-semibold">${totalAnnualSavings.toFixed(0)}/year</span> in savings
            </p>
          )}
        </div>
        ...
        <div className="flex justify-center mb-8">
          <ShareButton auditId={params.id} savings={totalMonthlySavings} />
        </div>
        ...
        {recommendations.length > 0 ? (
          <Card className="mb-8">
            <CardHeader>
              ...
              <CardDescription>
                {recommendations.length} optimization{recommendations.length !== 1 && 's'} found
              </CardDescription>
            </CardHeader>
          ...
          <Card className="mb-8">
            <CardContent className="py-12 text-center">
              <p className="text-lg text-slate-600">
                This team's AI spending is already optimized. Great job! 🎉
              </p>
            </CardContent>
          </Card>
        )}
        ...
        <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
          <CardHeader>
            ...
            <CardDescription>
              Get a free personalized analysis of your AI tool spending
            </CardDescription>
          </CardHeader>
          <CardContent>
            <a
        ...
        <footer className="text-center mt-12 text-sm text-slate-500">
          Powered by <a href="https://credex.rocks" className="underline">Credex</a>
        </footer>
      </div>
    </div>
  );
